...
Dist_30NN<-read_csv("https://raw.githubusercontent.com/JoonHwang-psu/MatlabEgoNet/refs/heads/main/MatlabEgoNet_HH_Dist_40NN_NoDuplicateRank.csv")
Dist_30NN_HHID<-Dist_30NN[,3:7]
Dist_30NN_wealth<-Dist_30NN[,c(3,5,6,10)]
Dist_30NN_distance<-Dist_30NN[,c(3,6,11)]

Dist_30NN_wide_HHID<-pivot_wider(Dist_30NN_HHID, names_from=NEAR_RANK, values_from=NEAR_FID)
Dist_30NN_wide_wealth<-pivot_wider(Dist_30NN_wealth, names_from=NEAR_RANK, values_from=HH_to_wealth)
Dist_30NN_wide_distance<-pivot_wider(Dist_30NN_distance, names_from=NEAR_RANK, values_from=NEAR_DIST)

n_HH<-nrow(Dist_30NN_wide_HHID)

Dist_30NN_wide_wealth$wealth_gini<-0
Dist_30NN_wide_distance$distance_mean<-0

hist(log(as.numeric(Dist_30NN_wide_wealth[1,3:32]), base=10))
hist(log(as.numeric(t(Dist_30NN_wide_wealth[,2])), base=10))

# calculating distance and wealth inequality among 30 neighbors
for (i in 1:n_HH){
  Dist_30NN_wide_wealth$wealth_gini[i]<-ineq(as.numeric(Dist_30NN_wide_wealth[i,2:32]), type= "Gini", na.rm=TRUE)
  Dist_30NN_wide_distance$distance_mean[i]<-mean(as.numeric(Dist_30NN_wide_distance[i,2:32]), na.rm=TRUE)
}

write.csv(Dist_30NN_wide_HHID, file="MatlabEgoNet_30NN_HHID.csv")
write.csv(Dist_30NN_wide_wealth, file="MatlabEgoNet_30NN_wealth.csv")
write.csv(Dist_30NN_wide_distance, file="MatlabEgoNet_30NN_distance.csv")

summary(Dist_30NN_wide_wealth$wealth_gini)
hist(Dist_30NN_wide_wealth$wealth_gini)
summary(Dist_30NN_wide_distance$distance_mean)
hist(Dist_30NN_wide_distance$distance_mean)

context_30NN<-merge(Dist_30NN_wide_HHID, Dist_30NN_wide_distance[,c(1,42)], by="HH_from")
context_30NN<-merge(context_30NN, Dist_30NN_wide_wealth[,c(1,43)], by="HH_from")

write.csv(context_30NN, file = "MatlabEgoNet_30NN_context.csv", row.names = TRUE)

##### 2. connecting ego networks to contextual variables

EgoNet_total <- read_csv("https://raw.githubusercontent.com/JoonHwang-psu/MatlabEgoNet/refs/heads/main/MatlabEgoNet_HH.csv")
colnames(context_30NN)[1]<-c("HHID")
colnames(EgoNet_total)[1]<-c("HHID")

EgoNet_total_context <- merge(EgoNet_total, context_30NN[,c(1,44,45)], by="HHID")
...
write.csv(formatted_table, "Netsize_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "3wayNetsize_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "Reciprocity_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "3wayReciprocity_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "LongDist_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "3wayLongDist_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "Local_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "3wayLocal_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "SuppIn_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "3waySuppIn_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "SuppOut_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "3waySuppOut_30NN.csv", row.names = FALSE)
...
formatted_table[is.na(formatted_table)] <- ""
print(formatted_table)
write.csv(formatted_table, "MainEffectModels_30NN.csv", row.names = FALSE)
...
variables <- c("sex", "muslim", "age_2018", "New_edu", "wealth1000", 
               "RecentEconRisk", "RecentHealthRisk", "RecentEnvRisk", "RecentSocRisk", "wealth_gini0.1", 
               "netsize", "mutual", "long", "local", "support_in", "support_out")

data_subset <- EgoNet_total_context %>%
  dplyr::select(all_of(variables))
data_subset <- data_subset %>%
  mutate(sex = as.numeric(as.character(sex)),
         muslim = as.numeric(as.character(muslim)))
...
stargazer(
  ...
  digits = 2,         
  out = "Descriptive_Statistics_EgoNet.html"  
)

dvs <- c("netsize", "mutual", "long", "local", "support_in", "support_out")
ivs <- c("RecentEconRisk", "RecentHealthRisk", "RecentEnvRisk", "RecentSocRisk", "wealth_gini")

par(mfrow = c(2, 3), mar=c(4,3,1,1))  

for (var in dvs) {
  hist(EgoNet_total_context[[var]], 
       main = "", 
       xlab = var, 
       col = "lightblue", 
       border = "black", 
...
for (var in ivs) {
  if (var %in% c("RecentEconRisk", "RecentHealthRisk", "RecentEnvRisk", "RecentSocRisk")) {
    hist(EgoNet_total_context[[var]],
         main="",
         xlab = var, 
         col = "lightblue", 
         border = "black", 
         breaks = seq(min(EgoNet_total_context[[var]], na.rm = TRUE) - 0.5, 
                      max(EgoNet_total_context[[var]], na.rm = TRUE) + 0.5, 
                      by = 1),
         xaxt = "n") 
    axis(1, at = seq(min(EgoNet_total_context[[var]], na.rm = TRUE), 
                     max(EgoNet_total_context[[var]], na.rm = TRUE), 
                     by = 1)) 
  } else {
    hist(EgoNet_total_context[[var]],
         main="",
...
cor_matrix_dvs <- cor(EgoNet_total_context[, dvs], use = "complete.obs")
cor_matrix_ivs <- cor(EgoNet_total_context[, ivs], use = "complete.obs")

stargazer(cor_matrix_dvs, type = "text", title = "Correlation Matrix of Dependent Variables", digits=4, out = "CorrMatrix_DV_EgoNet.html")
stargazer(cor_matrix_ivs, type = "text", title = "Correlation Matrix of Independent Variables", digits = 4, out = "CorrMatrix_IV_EgoNet.html")
